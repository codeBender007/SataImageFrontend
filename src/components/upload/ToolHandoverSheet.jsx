...
export default function ToolHandoverSheet({ records, details, onRecordChange, onDetailsChange, onSubmit, readOnly = false }) {
  ...
  const getDetail = (field) => details[field] || '';
  const handoverFields = ['problemAnalysis', 'why1', 'why2', 'why3', 'why4', 'why5', 'rootCause', 'action1', 'action2'];
        ...
        <div className="min-w-[1020px] p-2 sm:p-3">
          <table className="w-full border-collapse text-[10px] leading-tight text-slate-700 dark:text-slate-200">
            <tbody>
              ...
              {handoverRows.map((label, index) => <tr key={`${label}-${index}`}><td colSpan={2} className="border border-slate-400 px-2 py-1 dark:border-slate-600">{label}</td><td colSpan={3} className="border border-slate-400 p-0 dark:border-slate-600"><Field readOnly={readOnly} value={getDetail(handoverFields[index])} onChange={(value) => setDetail(handoverFields[index], value)} className="text-left" /></td><td colSpan={2} className="border border-slate-400 px-1 dark:border-slate-600">{index === 1 ? 'Handover check :' : index === 2 ? 'RM on line-Qty' : index === 3 ? 'Running Cavity' : index === 4 ? 'All Gauges online (Y/N)' : index === 5 ? 'Missing Gauges :' : index === 7 ? 'PDI Report/PV No.' : index === 8 ? 'Tool In/with line No.' : ''}</td><td className="border border-slate-400 p-0 dark:border-slate-600">{index > 1 && <Field readOnly={readOnly} value={getDetail(`handoverStart${index}`)} onChange={(value) => setDetail(`handoverStart${index}`, value)} />}</td><td className="border border-slate-400 p-0 dark:border-slate-600">{index > 1 && <Field readOnly={readOnly} value={getDetail(`handoverEnd${index}`)} onChange={(value) => setDetail(`handoverEnd${index}`, value)} />}</td><td colSpan={2} className="border border-slate-400 p-0 dark:border-slate-600"><Field readOnly={readOnly} value={getDetail(`handoverNote${index}`)} onChange={(value) => setDetail(`handoverNote${index}`, value)} /></td></tr>)}
              <tr><td colSpan={12} className="h-2 border-x border-slate-400 dark:border-slate-600" /></tr>
